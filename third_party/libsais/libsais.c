...
#define libsais_prefetchr(address) \
  __builtin_prefetch((const void*)(address), 0, 3)
#define libsais_prefetchw(address) \
  __builtin_prefetch((const void*)(address), 1, 3)
#elif defined(_M_IX86) || defined(_M_AMD64)
#include <intrin.h>
#define libsais_prefetchr(address) \
  _mm_prefetch((const void*)(address), _MM_HINT_T0)
...
static void* libsais_alloc_aligned(size_t size, size_t alignment) {
  ...
  if (address != NULL) {
    void* aligned_address = libsais_align_up(
        (void*)((ptrdiff_t)address + (ptrdiff_t)(sizeof(short))), alignment);
...
static LIBSAIS_CONTEXT* libsais_create_ctx_main(sa_sint_t threads) {
  ...
  sa_sint_t* RESTRICT buckets = (sa_sint_t*)libsais_alloc_aligned(
      (size_t)8 * ALPHABET_SIZE * sizeof(sa_sint_t), 4096);
  LIBSAIS_THREAD_STATE* RESTRICT thread_state =
...
static void libsais_accumulate_counts_s32(sa_sint_t* RESTRICT buckets,
  ...
  while (num_buckets >= 9) {
    libsais_accumulate_counts_s32_9(buckets - (num_buckets - 9) * bucket_stride,
                                    bucket_size, bucket_stride);
    num_buckets -= 8;
  }
...
static void libsais_flip_suffix_markers_omp(sa_sint_t* RESTRICT SA, sa_sint_t l,
                                            sa_sint_t threads) {
#if defined(LIBSAIS_OPENMP)
...
static void libsais_gather_lms_suffixes_8u(const uint8_t* RESTRICT T,
  ...
  if (omp_block_size > 0) {
    ...
    for (i = omp_block_start + omp_block_size - 2, j = omp_block_start + 3;
      ...
      c1 = T[i - 0];
      f1 = (fast_uint_t)(c1 > (c0 - (fast_sint_t)(f0)));
      SA[m] = (sa_sint_t)(i + 1);
      m -= (f1 & ~f0);
      c0 = T[i - 1];
      f0 = (fast_uint_t)(c0 > (c1 - (fast_sint_t)(f1)));
      SA[m] = (sa_sint_t)(i - 0);
      m -= (f0 & ~f1);
      c1 = T[i - 2];
      f1 = (fast_uint_t)(c1 > (c0 - (fast_sint_t)(f0)));
...
static sa_sint_t libsais_gather_compacted_lms_suffixes_32s(
  ...
  for (; i >= 3; i -= 4) {
    ...
    f1 = (fast_uint_t)(c1 > (c0 - (fast_sint_t)(f0)));
    SA[m] = i + 1;
    m -= (sa_sint_t)(f1 & ~f0 & (c0 >= 0));
    c0 = T[i - 1];
    f0 = (fast_uint_t)(c0 > (c1 - (fast_sint_t)(f1)));
    SA[m] = i - 0;
    m -= (sa_sint_t)(f0 & ~f1 & (c1 >= 0));
    c1 = T[i - 2];
    f1 = (fast_uint_t)(c1 > (c0 - (fast_sint_t)(f0)));
    SA[m] = i - 1;
    m -= (sa_sint_t)(f1 & ~f0 & (c0 >= 0));
...
static sa_sint_t libsais_count_and_gather_lms_suffixes_32s_4k_fs_omp(
    const sa_sint_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n,
    sa_sint_t k, sa_sint_t* RESTRICT buckets, sa_sint_t local_buckets,
    sa_sint_t threads, LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  sa_sint_t m = 0;
...
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
#endif
  {
    ...
    if (omp_num_threads == 1) {
      ...
      fast_sint_t free_space =
          local_buckets ? LIBSAIS_LOCAL_BUFFER_SIZE : (buckets - &SA[n]);
      fast_sint_t bucket_stride =
          libsais_get_bucket_stride(free_space, bucket_size, omp_num_threads);
...
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
#endif
  {
    ...
    if (omp_num_threads == 1) {
      ...
      fast_sint_t bucket_size = 2 * (fast_sint_t)k;
      fast_sint_t free_space =
          local_buckets ? LIBSAIS_LOCAL_BUFFER_SIZE
                        : (buckets - &SA[(fast_sint_t)n + (fast_sint_t)n]);
      fast_sint_t bucket_stride =
          libsais_get_bucket_stride(free_space, bucket_size, omp_num_threads);
...
#endif
    if (omp_num_threads == 1) {
      m = libsais_count_and_gather_lms_suffixes_32s_4k(T, SA, n, k, buckets, 0,
                                                       n);
    }
#if defined(LIBSAIS_OPENMP)
...
#endif
    if (omp_num_threads == 1) {
      m = libsais_count_and_gather_lms_suffixes_32s_2k(T, SA, n, k, buckets, 0,
                                                       n);
    }
#if defined(LIBSAIS_OPENMP)
    else if (omp_thread_num == 0) {
...
#endif
    if (omp_num_threads == 1) {
      m = libsais_count_and_gather_compacted_lms_suffixes_32s_2k(T, SA, n, k,
                                                                 buckets, 0, n);
    }
#if defined(LIBSAIS_OPENMP)
    else if (omp_thread_num == 0) {
...
static sa_sint_t libsais_count_and_gather_lms_suffixes_32s_4k_omp(
  ...
  sa_sint_t max_threads =
      (sa_sint_t)(free_space / ((4 * (fast_sint_t)k + 15) & (-16)));
  ...
  if (max_threads > 1 && n >= 65536 && n / k >= 2) {
    ...
    m = libsais_count_and_gather_lms_suffixes_32s_4k_fs_omp(
        T, SA, n, k, buckets, local_buckets, max_threads > 2 ? max_threads : 2,
        thread_state);
  } else
#else
  UNUSED(local_buckets);
...
#endif
  {
    m = libsais_count_and_gather_lms_suffixes_32s_4k_nofs_omp(T, SA, n, k,
                                                              buckets, threads);
  }
...
static sa_sint_t libsais_count_and_gather_lms_suffixes_32s_2k_omp(
  ...
  fast_sint_t free_space =
      local_buckets ? LIBSAIS_LOCAL_BUFFER_SIZE : (buckets - &SA[n]);
  sa_sint_t max_threads =
      (sa_sint_t)(free_space / ((2 * (fast_sint_t)k + 15) & (-16)));
  if (max_threads > threads) {
    max_threads = threads;
  }
...
#endif
  {
    m = libsais_count_and_gather_lms_suffixes_32s_2k_nofs_omp(T, SA, n, k,
                                                              buckets, threads);
  }
...
static void libsais_count_suffixes_32s(const sa_sint_t* RESTRICT T, sa_sint_t n,
                                       sa_sint_t k,
                                       sa_sint_t* RESTRICT buckets) {
  const fast_sint_t prefetch_distance = 64;
...
static sa_sint_t libsais_initialize_buckets_start_and_end_8u(
  ...
  if (freq != NULL) {
    ...
    for (i = BUCKETS_INDEX4(0, 0), j = 0;
      ...
      sa_sint_t total = buckets[i + BUCKETS_INDEX4(0, 0)] +
...
static void libsais_initialize_buckets_start_and_end_32s_6k(
  ...
  for (i = BUCKETS_INDEX4(0, 0), j = 0;
    ...
    bucket_start[j] = sum;
    sum +=
        buckets[i + BUCKETS_INDEX4(0, 0)] + buckets[i + BUCKETS_INDEX4(0, 1)] +
...
static void libsais_initialize_buckets_start_and_end_32s_4k(
  ...
  for (i = BUCKETS_INDEX2(0, 0), j = 0;
    ...
    bucket_start[j] = sum;
    sum +=
        buckets[i + BUCKETS_INDEX2(0, 0)] + buckets[i + BUCKETS_INDEX2(0, 1)];
    bucket_end[j] = sum;
  }
}

static void libsais_initialize_buckets_end_32s_2k(sa_sint_t k,
                                                  sa_sint_t* RESTRICT buckets) {
  fast_sint_t i;
  sa_sint_t sum0 = 0;
...
static void libsais_initialize_buckets_end_32s_1k(sa_sint_t k,
                                                  sa_sint_t* RESTRICT buckets) {
  fast_sint_t i;
  sa_sint_t sum = 0;
  for (i = 0; i <= (fast_sint_t)k - 1; i += 1) {
...
static sa_sint_t libsais_initialize_buckets_for_lms_suffixes_radix_sort_8u(
...

  {
    ...
    for (i = BUCKETS_INDEX4(0, 0), j = BUCKETS_INDEX2(0, 0);
      ...
      sum +=
          buckets[i + BUCKETS_INDEX4(0, 1)] + buckets[i + BUCKETS_INDEX4(0, 3)];
      temp_bucket[j] = sum;
    }
...
static void libsais_radix_sort_lms_suffixes_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t m,
    sa_sint_t flags, sa_sint_t* RESTRICT buckets, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
...
static void libsais_radix_sort_lms_suffixes_32s_2k(
  ...
  for (i = omp_block_start + omp_block_size - 1,
    ...
    libsais_prefetchw(
        &induction_bucket[BUCKETS_INDEX2(T[SA[i - prefetch_distance - 0]], 0)]);
    libsais_prefetchw(
        &induction_bucket[BUCKETS_INDEX2(T[SA[i - prefetch_distance - 1]], 0)]);
    libsais_prefetchw(
        &induction_bucket[BUCKETS_INDEX2(T[SA[i - prefetch_distance - 2]], 0)]);
    libsais_prefetchw(
        &induction_bucket[BUCKETS_INDEX2(T[SA[i - prefetch_distance - 3]], 0)]);
...
static void libsais_radix_sort_lms_suffixes_32s_6k_block_sort(
    sa_sint_t* RESTRICT induction_bucket, LIBSAIS_THREAD_CACHE* RESTRICT cache,
    fast_sint_t omp_block_start, fast_sint_t omp_block_size) {
  const fast_sint_t prefetch_distance = 64;
...
#pragma omp parallel num_threads(threads) if (threads > 1 && \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_radix_sort_lms_suffixes_32s_6k(T, SA, induction_bucket,
                                             omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
#pragma omp parallel num_threads(threads) if (threads > 1 && \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_radix_sort_lms_suffixes_32s_2k(T, SA, induction_bucket,
                                             omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
static void libsais_radix_sort_lms_suffixes_32s_6k_omp(
  ...
  if (threads == 1 || m < 65536) {
    ...
    for (block_start = 0; block_start < (fast_sint_t)m - 1;
      ...
      block_end =
          block_start + (fast_sint_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE;
      if (block_end >= m) {
...
static void libsais_radix_sort_set_markers_32s_4k(
  ...
  for (i = omp_block_start,
    ...
    libsais_prefetchw(
        &SA[induction_bucket[BUCKETS_INDEX2(i + prefetch_distance + 0, 0)]]);
    libsais_prefetchw(
        &SA[induction_bucket[BUCKETS_INDEX2(i + prefetch_distance + 1, 0)]]);
    libsais_prefetchw(
        &SA[induction_bucket[BUCKETS_INDEX2(i + prefetch_distance + 2, 0)]]);
    libsais_prefetchw(
        &SA[induction_bucket[BUCKETS_INDEX2(i + prefetch_distance + 3, 0)]]);
...
static void libsais_initialize_buckets_for_partial_sorting_8u(
  ...
  for (i = BUCKETS_INDEX4(0, 0), j = BUCKETS_INDEX2(0, 0);
    ...
    sum0 +=
        buckets[i + BUCKETS_INDEX4(0, 0)] + buckets[i + BUCKETS_INDEX4(0, 2)];
...
static void libsais_initialize_buckets_for_partial_sorting_32s_6k(
  ...
  for (first_lms_suffix = T[first_lms_suffix], i = BUCKETS_INDEX4(0, 0),
      j = BUCKETS_INDEX2(0, 0);
       i != BUCKETS_INDEX4((fast_sint_t)first_lms_suffix, 0);
       i += BUCKETS_INDEX4(1, 0), j += BUCKETS_INDEX2(1, 0)) {
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_8u(
  ...
  for (i = omp_block_start,
    ...
    SA[induction_bucket[v0]++] =
        (p0 - 1) |
        (sa_sint_t)((sa_uint_t)(distinct_names[v0] != d) << (SAINT_BIT - 1));
    distinct_names[v0] = d;
    ...
    SA[induction_bucket[v1]++] =
        (p1 - 1) |
        (sa_sint_t)((sa_uint_t)(distinct_names[v1] != d) << (SAINT_BIT - 1));
    distinct_names[v1] = d;
  }
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    SA[induction_bucket[v]++] =
        (p - 1) |
        (sa_sint_t)((sa_uint_t)(distinct_names[v] != d) << (SAINT_BIT - 1));
    distinct_names[v] = d;
  }
...
static void libsais_partial_sorting_scan_left_to_right_8u_block_prepare(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t k,
...
#pragma omp parallel num_threads(                                             \
        threads) if (threads > 1 && block_size >= 64 * (k > 256 ? k : 256) && \
                         omp_get_dynamic() == 0)
#endif
  {
#if defined(LIBSAIS_OPENMP)
    ...
    if (omp_num_threads == 1) {
    ...
    else {
      {
        libsais_partial_sorting_scan_left_to_right_8u_block_prepare(
            T, SA, k, thread_state[omp_thread_num].state.buckets,
...
#pragma omp master
      {
        ...
        for (t = 0; t < omp_num_threads; ++t) {
          ...
          for (d -= 1, c = 0; c < 2 * k; c += 1) {
            sa_sint_t A = distinct_names[c], B = temp_distinct_names[c],
                      D = B + d;
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t k,
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          for (; block_start < block_end; block_start += 1) {
            ...
            sa_sint_t v = BUCKETS_INDEX2(T[p - 1], T[p - 2] >= T[p - 1]);
            SA[induction_bucket[v]++] =
                (p - 1) | (sa_sint_t)((sa_uint_t)(distinct_names[v] != d)
                                      << (SAINT_BIT - 1));
            distinct_names[v] = d;
          }
        } else {
          d = libsais_partial_sorting_scan_left_to_right_8u_block_omp(
              T, SA, k, buckets, d, block_start, block_size, threads,
              thread_state);
          block_start = block_end;
        }
      }
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_32s_6k(
  ...
  for (i = omp_block_start,
    ...
    SA[buckets[v2]++] = (p2 - 1) | (sa_sint_t)((sa_uint_t)(buckets[2 + v2] != d)
                                               << (SAINT_BIT - 1));
    buckets[2 + v2] = d;
    ...
    SA[buckets[v3]++] = (p3 - 1) | (sa_sint_t)((sa_uint_t)(buckets[2 + v3] != d)
                                               << (SAINT_BIT - 1));
    buckets[2 + v3] = d;
  }
  ...
  for (j += 2 * prefetch_distance + 1; i < j; i += 1) {
    ...
    SA[buckets[v]++] = (p - 1) | (sa_sint_t)((sa_uint_t)(buckets[2 + v] != d)
                                             << (SAINT_BIT - 1));
    buckets[2 + v] = d;
  }
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_32s_4k(
  ...
  for (i = omp_block_start,
    ...
    const sa_sint_t* Ts0 = &T[s0 > 0 ? s0 & ~SUFFIX_GROUP_MARKER : 2];
    libsais_prefetchr(Ts0 - 1);
    libsais_prefetchr(Ts0 - 2);
    sa_sint_t s1 = SA[i + 2 * prefetch_distance + 1];
    const sa_sint_t* Ts1 = &T[s1 > 0 ? s1 & ~SUFFIX_GROUP_MARKER : 2];
    libsais_prefetchr(Ts1 - 1);
    libsais_prefetchr(Ts1 - 2);
    ...
    if (p0 > 0) {
      ...
      SA[induction_bucket[T[p0 - 1]]++] =
          (p0 - 1) |
          (sa_sint_t)((sa_uint_t)(T[p0 - 2] < T[p0 - 1]) << (SAINT_BIT - 1)) |
    ...
    if (p1 > 0) {
      ...
      SA[induction_bucket[T[p1 - 1]]++] =
          (p1 - 1) |
          (sa_sint_t)((sa_uint_t)(T[p1 - 2] < T[p1 - 1]) << (SAINT_BIT - 1)) |
  ...
  for (j += 2 * prefetch_distance + 1; i < j; i += 1) {
    ...
    if (p > 0) {
      ...
      SA[induction_bucket[T[p - 1]]++] =
          (p - 1) |
          (sa_sint_t)((sa_uint_t)(T[p - 2] < T[p - 1]) << (SAINT_BIT - 1)) |
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k(
  ...
  for (i = omp_block_start,
    ...
    sa_sint_t s0 = SA[i + 2 * prefetch_distance + 0];
    const sa_sint_t* Ts0 = &T[s0 > 0 ? s0 : 1];
    libsais_prefetchr(Ts0 - 1);
    sa_sint_t s1 = SA[i + 2 * prefetch_distance + 1];
    const sa_sint_t* Ts1 = &T[s1 > 0 ? s1 : 1];
    libsais_prefetchr(Ts1 - 1);
    sa_sint_t s2 = SA[i + 1 * prefetch_distance + 0];
    if (s2 > 0) {
      libsais_prefetchw(&induction_bucket[T[s2 - 1]]);
      libsais_prefetchr(&T[s2] - 2);
    }
    sa_sint_t s3 = SA[i + 1 * prefetch_distance + 1];
    if (s3 > 0) {
      libsais_prefetchw(&induction_bucket[T[s3 - 1]]);
      libsais_prefetchr(&T[s3] - 2);
    }
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k_block_gather(
  ...
  for (i = omp_block_start,
    ...
    sa_sint_t s0 = SA[i + prefetch_distance + 0];
    const sa_sint_t* Ts0 = &T[s0 > 0 ? s0 : 2];
    libsais_prefetchr(Ts0 - 1);
    libsais_prefetchr(Ts0 - 2);
    sa_sint_t s1 = SA[i + prefetch_distance + 1];
    const sa_sint_t* Ts1 = &T[s1 > 0 ? s1 : 2];
    libsais_prefetchr(Ts1 - 1);
    libsais_prefetchr(Ts1 - 2);
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    sa_sint_t symbol = SAINT_MIN, p = SA[i];
    if (p > 0) {
      cache[i].index = (p - 1) | (sa_sint_t)((sa_uint_t)(T[p - 2] < T[p - 1])
                                             << (SAINT_BIT - 1));
      symbol = T[p - 1];
      p = 0;
    }
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_32s_6k_block_sort(
  ...
  for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j;
    ...
    cache[i + 0].index =
        (p0 - 1) |
        (sa_sint_t)((sa_uint_t)(buckets[2 + v0] != d) << (SAINT_BIT - 1));
    buckets[2 + v0] = d;
    ...
    cache[i + 1].index =
        (p1 - 1) |
        (sa_sint_t)((sa_uint_t)(buckets[2 + v1] != d) << (SAINT_BIT - 1));
    buckets[2 + v1] = d;
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    cache[i].index = (p - 1) | (sa_sint_t)((sa_uint_t)(buckets[2 + v] != d)
                                           << (SAINT_BIT - 1));
    buckets[2 + v] = d;
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_32s_4k_block_sort(
  ...
  for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j;
    ...
    const sa_sint_t* Is0 = &induction_bucket[s0 > 0 ? s0 >> 1 : 0];
    libsais_prefetchw(Is0);
    const sa_sint_t* Ds0 = &distinct_names[s0 > 0 ? s0 : 0];
    libsais_prefetchw(Ds0);
    sa_sint_t s1 = cache[i + prefetch_distance + 1].symbol;
    const sa_sint_t* Is1 = &induction_bucket[s1 > 0 ? s1 >> 1 : 0];
    libsais_prefetchw(Is1);
    const sa_sint_t* Ds1 = &distinct_names[s1 > 0 ? s1 : 0];
    libsais_prefetchw(Ds1);
    ...
    if (v0 >= 0) {
      ...
      cache[i + 0].index =
          (p0 - 1) | (sa_sint_t)((sa_uint_t)v0 << (SAINT_BIT - 1)) |
          ((sa_sint_t)(distinct_names[v0] != d) << (SUFFIX_GROUP_BIT - 1));
      distinct_names[v0] = d;
      if (cache[i + 0].symbol < omp_block_end) {
    ...
    if (v1 >= 0) {
      ...
      cache[i + 1].index =
          (p1 - 1) | (sa_sint_t)((sa_uint_t)v1 << (SAINT_BIT - 1)) |
          ((sa_sint_t)(distinct_names[v1] != d) << (SUFFIX_GROUP_BIT - 1));
      distinct_names[v1] = d;
      if (cache[i + 1].symbol < omp_block_end) {
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    if (v >= 0) {
      ...
      cache[i].index =
          (p - 1) | (sa_sint_t)((sa_uint_t)v << (SAINT_BIT - 1)) |
          ((sa_sint_t)(distinct_names[v] != d) << (SUFFIX_GROUP_BIT - 1));
      distinct_names[v] = d;
      if (cache[i].symbol < omp_block_end) {
...
static void libsais_partial_sorting_scan_left_to_right_32s_1k_block_sort(
  ...
  for (i = omp_block_start, j = omp_block_end - prefetch_distance - 1; i < j;
    ...
    sa_sint_t s0 = cache[i + prefetch_distance + 0].symbol;
    const sa_sint_t* Is0 = &induction_bucket[s0 > 0 ? s0 : 0];
    libsais_prefetchw(Is0);
    sa_sint_t s1 = cache[i + prefetch_distance + 1].symbol;
    const sa_sint_t* Is1 = &induction_bucket[s1 > 0 ? s1 : 0];
    libsais_prefetchw(Is1);
    ...
    if (v0 >= 0) {
      ...
      if (cache[i + 0].symbol < omp_block_end) {
        ...
        if (np > 0) {
          cache[ni].index =
              (np - 1) | (sa_sint_t)((sa_uint_t)(T[np - 2] < T[np - 1])
                                     << (SAINT_BIT - 1));
...
static sa_sint_t libsais_partial_sorting_scan_left_to_right_32s_4k_omp(
  ...
  SA[induction_bucket[T[n - 1]]++] =
      (n - 1) |
      (sa_sint_t)((sa_uint_t)(T[n - 2] < T[n - 1]) << (SAINT_BIT - 1)) |
      SUFFIX_GROUP_MARKER;
  distinct_names[BUCKETS_INDEX2(T[n - 1], T[n - 2] < T[n - 1])] = ++d;
...
static void libsais_partial_sorting_shift_markers_32s_4k(sa_sint_t* RESTRICT SA,
                                                         sa_sint_t n) {
  const fast_sint_t prefetch_distance = 64;
...
static sa_sint_t libsais_partial_gsa_scan_right_to_left_8u(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA,
    sa_sint_t* RESTRICT buckets, sa_sint_t d, fast_sint_t omp_block_start,
    fast_sint_t omp_block_size) {
  const fast_sint_t prefetch_distance = 64;
...
static void libsais_partial_gsa_scan_right_to_left_8u_block_place(
    sa_sint_t* RESTRICT SA, sa_sint_t* RESTRICT buckets,
    LIBSAIS_THREAD_CACHE* RESTRICT cache, fast_sint_t count, sa_sint_t d) {
  const fast_sint_t prefetch_distance = 64;
...
static sa_sint_t libsais_partial_gsa_scan_right_to_left_8u_block_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t k,
    sa_sint_t* RESTRICT buckets, sa_sint_t d, fast_sint_t block_start,
    fast_sint_t block_size, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
...
#pragma omp parallel num_threads(                                             \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      d = libsais_partial_gsa_scan_right_to_left_8u(
          T, SA, buckets, d, omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
#pragma omp barrier
      ...
      {
        libsais_partial_gsa_scan_right_to_left_8u_block_place(
            SA, thread_state[omp_thread_num].state.buckets,
            thread_state[omp_thread_num].state.cache,
            thread_state[omp_thread_num].state.count,
            (sa_sint_t)thread_state[omp_thread_num].state.position);
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          d = libsais_partial_sorting_scan_right_to_left_8u_block_omp(
              T, SA, k, buckets, d, block_end + 1, block_size, threads,
              thread_state);
          block_start = block_end;
        }
      }
...
static void libsais_partial_gsa_scan_right_to_left_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t k,
    sa_sint_t* RESTRICT buckets, sa_sint_t first_lms_suffix,
    sa_sint_t left_suffixes_count, sa_sint_t d, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  ...
  if (threads == 1 || (scan_end - scan_start) < 65536) {
    libsais_partial_gsa_scan_right_to_left_8u(T, SA, buckets, d, scan_start,
                                              scan_end - scan_start);
  }
#if defined(LIBSAIS_OPENMP)
  else {
    ...
    for (block_start = scan_end - 1; block_start >= scan_start;) {
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          d = libsais_partial_gsa_scan_right_to_left_8u_block_omp(
              T, SA, k, buckets, d, block_end + 1, block_size, threads,
              thread_state);
          block_start = block_end;
        }
...
static fast_sint_t libsais_partial_sorting_gather_lms_suffixes_32s_4k(
  ...
  for (i = omp_block_start, j = omp_block_start + omp_block_size - 3,
    ...
    sa_uint_t s0 = (sa_uint_t)SA[i + 0];
    SA[l] = (sa_sint_t)((s0 - (sa_uint_t)SUFFIX_GROUP_MARKER) &
                        (sa_uint_t)(~SUFFIX_GROUP_MARKER));
    l += ((sa_sint_t)s0 < 0);
    sa_uint_t s1 = (sa_uint_t)SA[i + 1];
    SA[l] = (sa_sint_t)((s1 - (sa_uint_t)SUFFIX_GROUP_MARKER) &
                        (sa_uint_t)(~SUFFIX_GROUP_MARKER));
    l += ((sa_sint_t)s1 < 0);
    sa_uint_t s2 = (sa_uint_t)SA[i + 2];
    SA[l] = (sa_sint_t)((s2 - (sa_uint_t)SUFFIX_GROUP_MARKER) &
                        (sa_uint_t)(~SUFFIX_GROUP_MARKER));
    l += ((sa_sint_t)s2 < 0);
    sa_uint_t s3 = (sa_uint_t)SA[i + 3];
    SA[l] = (sa_sint_t)((s3 - (sa_uint_t)SUFFIX_GROUP_MARKER) &
                        (sa_uint_t)(~SUFFIX_GROUP_MARKER));
    l += ((sa_sint_t)s3 < 0);
  }

  for (j += 3; i < j; i += 1) {
    sa_uint_t s = (sa_uint_t)SA[i];
    SA[l] = (sa_sint_t)((s - (sa_uint_t)SUFFIX_GROUP_MARKER) &
                        (sa_uint_t)(~SUFFIX_GROUP_MARKER));
    l += ((sa_sint_t)s < 0);
  }
...
static void libsais_induce_partial_order_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t k,
    sa_sint_t flags, sa_sint_t* RESTRICT buckets, sa_sint_t first_lms_suffix,
    sa_sint_t left_suffixes_count, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  memset(&buckets[2 * ALPHABET_SIZE], 0,
         (size_t)2 * ALPHABET_SIZE * sizeof(sa_sint_t));

  if (flags & LIBSAIS_FLAGS_GSA) {
    buckets[4 * ALPHABET_SIZE + BUCKETS_INDEX2(0, 1)] =
        buckets[4 * ALPHABET_SIZE + BUCKETS_INDEX2(1, 1)] - 1;
    libsais_flip_suffix_markers_omp(
        SA, buckets[4 * ALPHABET_SIZE + BUCKETS_INDEX2(0, 1)], threads);
  }

  sa_sint_t d = libsais_partial_sorting_scan_left_to_right_8u_omp(
      T, SA, n, k, buckets, left_suffixes_count, 0, threads, thread_state);
  libsais_partial_sorting_shift_markers_8u_omp(SA, n, buckets, threads);

  if (flags & LIBSAIS_FLAGS_GSA) {
    libsais_partial_gsa_scan_right_to_left_8u_omp(
        T, SA, n, k, buckets, first_lms_suffix, left_suffixes_count, d, threads,
        thread_state);

    if (T[first_lms_suffix] == 0) {
      memmove(&SA[1], &SA[0],
              (size_t)(buckets[BUCKETS_INDEX2(1, 1)] - 1) * sizeof(sa_sint_t));
...
static void libsais_induce_partial_order_32s_4k_omp(
  ...
  libsais_partial_sorting_scan_right_to_left_32s_4k_omp(T, SA, n, k, buckets, d,
...
static void libsais_induce_partial_order_32s_2k_omp(
  ...
  libsais_partial_sorting_scan_left_to_right_32s_1k_omp(
      T, SA, n, &buckets[1 * (fast_sint_t)k], threads, thread_state);
  libsais_partial_sorting_scan_right_to_left_32s_1k_omp(
      T, SA, n, &buckets[0 * (fast_sint_t)k], threads, thread_state);
...
static void libsais_induce_partial_order_32s_1k_omp(
  ...
  libsais_partial_sorting_scan_left_to_right_32s_1k_omp(T, SA, n, buckets,
                                                        threads, thread_state);

  libsais_count_suffixes_32s(T, n, k, buckets);
  libsais_initialize_buckets_end_32s_1k(k, buckets);
  libsais_partial_sorting_scan_right_to_left_32s_1k_omp(T, SA, n, buckets,
                                                        threads, thread_state);
...
static fast_sint_t libsais_gather_marked_lms_suffixes(
...
#pragma omp barrier

      {
        ...
        libsais_renumber_lms_suffixes_8u(SA, m, (sa_sint_t)count,
                                         omp_block_start, omp_block_size);
      }
    }
#endif
...
static void libsais_gather_marked_lms_suffixes_omp(
...
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 131072)
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_gather_marked_lms_suffixes(SA, m,
        ...
        if (omp_thread_num < omp_num_threads - 1) {
          thread_state[omp_thread_num].state.position =
              libsais_gather_marked_lms_suffixes(
                  SA, m, (fast_sint_t)m + omp_block_start + omp_block_size,
                  omp_block_start, omp_block_size);
          thread_state[omp_thread_num].state.count =
          ...
          thread_state[omp_thread_num].state.position =
              libsais_gather_marked_lms_suffixes(
                  SA, m, (fast_sint_t)n + (fast_sint_t)fs, omp_block_start,
                  omp_block_size);
          thread_state[omp_thread_num].state.count =
              (fast_sint_t)n + (fast_sint_t)fs -
              thread_state[omp_thread_num].state.position;
...
static sa_sint_t libsais_renumber_and_gather_lms_suffixes_omp(
    sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t m, sa_sint_t fs,
    sa_sint_t threads, LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
...
static sa_sint_t libsais_renumber_and_mark_distinct_lms_suffixes_32s_1k_omp(
...

  {
    ...
    for (i = 1, j = m - prefetch_distance - 1; i < j; i += 2) {
      ...
      libsais_prefetchw(&SAm[((sa_uint_t)SA[i + prefetch_distance + 0]) >> 1]);
      libsais_prefetchr(&T[((sa_uint_t)SA[i + prefetch_distance + 0])]);
      libsais_prefetchw(&SAm[((sa_uint_t)SA[i + prefetch_distance + 1]) >> 1]);
      libsais_prefetchr(&T[((sa_uint_t)SA[i + prefetch_distance + 1])]);
...
static void libsais_place_lms_suffixes_interval_8u(
    sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t m, sa_sint_t flags,
    sa_sint_t* RESTRICT buckets) {
  if (flags & LIBSAIS_FLAGS_GSA) {
    buckets[7 * ALPHABET_SIZE]--;
  }
...
static void libsais_final_bwt_scan_left_to_right_8u(
  ...
  for (i = omp_block_start,
    ...
    sa_sint_t s0 = SA[i + prefetch_distance + 0];
    const uint8_t* Ts0 = &T[s0 > 0 ? s0 : 2];
    libsais_prefetchr(Ts0 - 1);
    libsais_prefetchr(Ts0 - 2);
    sa_sint_t s1 = SA[i + prefetch_distance + 1];
    const uint8_t* Ts1 = &T[s1 > 0 ? s1 : 2];
    libsais_prefetchr(Ts1 - 1);
    libsais_prefetchr(Ts1 - 2);
    ...
    if (p0 > 0) {
      ...
      SA[induction_bucket[T[p0]]++] =
          p0 |
          (sa_sint_t)((sa_uint_t)(T[p0 - (p0 > 0)] < T[p0]) << (SAINT_BIT - 1));
    }
    sa_sint_t p1 = SA[i + 1];
    SA[i + 1] = p1 & SAINT_MAX;
    if (p1 > 0) {
      ...
      SA[induction_bucket[T[p1]]++] =
          p1 |
          (sa_sint_t)((sa_uint_t)(T[p1 - (p1 > 0)] < T[p1]) << (SAINT_BIT - 1));
    }
  }
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    if (p > 0) {
      ...
      SA[induction_bucket[T[p]]++] =
          p |
          (sa_sint_t)((sa_uint_t)(T[p - (p > 0)] < T[p]) << (SAINT_BIT - 1));
    }
  }
}
...
static fast_sint_t libsais_final_bwt_scan_left_to_right_8u_block_prepare(
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    if (p > 0) {
      ...
      cache[count++].index = p | (sa_sint_t)((sa_uint_t)(T[p - (p > 0)] < T[p])
                                             << (SAINT_BIT - 1));
    }
  }
...
static void libsais_final_sorting_scan_left_to_right_32s_block_gather(
  ...
  for (j += prefetch_distance + 1; i < j; i += 1) {
    ...
    if (p > 0) {
      p--;
      cache[i].index = p | (sa_sint_t)((sa_uint_t)(T[p - (p > 0)] < T[p])
                                       << (SAINT_BIT - 1));
      symbol = T[p];
    }
    cache[i].symbol = symbol;
    ...
    if (v0 >= 0) {
      ...
      if (cache[i + 0].symbol < omp_block_end) {
        ...
        if (np > 0) {
          np--;
          cache[ni].index =
              np | (sa_sint_t)((sa_uint_t)(T[np - (np > 0)] < T[np])
                               << (SAINT_BIT - 1));
...
#pragma omp parallel num_threads(                                             \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_final_bwt_scan_left_to_right_8u(T, SA, induction_bucket,
                                              omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_final_bwt_aux_scan_left_to_right_8u(
          T, SA, rm, I, induction_bucket, omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
      {
...
static void libsais_final_bwt_scan_left_to_right_8u_omp(
    ...
    sa_sint_t k, sa_sint_t* RESTRICT induction_bucket, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  SA[induction_bucket[T[(sa_sint_t)n - 1]]++] =
      ((sa_sint_t)n - 1) |
      (sa_sint_t)((sa_uint_t)(T[(sa_sint_t)n - 2] < T[(sa_sint_t)n - 1])
                  << (SAINT_BIT - 1));
        ...
        if (block_size < 32) {
          for (; block_start < block_end; block_start += 1) {
            ...
            if (p > 0) {
              ...
              SA[induction_bucket[T[p]]++] =
                  p | (sa_sint_t)((sa_uint_t)(T[p - (p > 0)] < T[p])
                                  << (SAINT_BIT - 1));
            }
          }
        } else {
          libsais_final_bwt_scan_left_to_right_8u_block_omp(
              T, SA, k, induction_bucket, block_start, block_size, threads,
              thread_state);
          block_start = block_end;
        }
      }
...
static void libsais_final_bwt_aux_scan_left_to_right_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, fast_sint_t n,
    sa_sint_t k, sa_sint_t rm, sa_sint_t* RESTRICT I,
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          libsais_final_bwt_aux_scan_left_to_right_8u_block_omp(
              T, SA, k, rm, I, induction_bucket, block_start, block_size,
              threads, thread_state);
          block_start = block_end;
        }
      }
...
static void libsais_final_gsa_scan_right_to_left_8u(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA,
    sa_sint_t* RESTRICT induction_bucket, fast_sint_t omp_block_start,
    fast_sint_t omp_block_size) {
  const fast_sint_t prefetch_distance = 64;
...
static void libsais_final_gsa_scan_right_to_left_8u_block_place(
    sa_sint_t* RESTRICT SA, sa_sint_t* RESTRICT buckets,
    LIBSAIS_THREAD_CACHE* RESTRICT cache, fast_sint_t count) {
  const fast_sint_t prefetch_distance = 64;
  ...
  for (i = 0, j = count - 3; i < j; i += 4) {
    ...
    if (cache[i + 0].symbol > 0) {
      SA[--buckets[cache[i + 0].symbol]] = cache[i + 0].index;
    }
    if (cache[i + 1].symbol > 0) {
      SA[--buckets[cache[i + 1].symbol]] = cache[i + 1].index;
    }
    if (cache[i + 2].symbol > 0) {
      SA[--buckets[cache[i + 2].symbol]] = cache[i + 2].index;
    }
    if (cache[i + 3].symbol > 0) {
      SA[--buckets[cache[i + 3].symbol]] = cache[i + 3].index;
    }
  }
...
#pragma omp parallel num_threads(                                             \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_final_bwt_scan_right_to_left_8u(T, SA, induction_bucket,
                                              omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
static void libsais_final_gsa_scan_right_to_left_8u_block_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t k,
    sa_sint_t* RESTRICT induction_bucket, fast_sint_t block_start,
    fast_sint_t block_size, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
...
#pragma omp parallel num_threads(                                             \
...
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_final_gsa_scan_right_to_left_8u(T, SA, induction_bucket,
                                              omp_block_start, omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
#pragma omp barrier
      ...
      {
        libsais_final_gsa_scan_right_to_left_8u_block_place(
            SA, thread_state[omp_thread_num].state.buckets,
            thread_state[omp_thread_num].state.cache,
            thread_state[omp_thread_num].state.count);
      }
...
static sa_sint_t libsais_final_bwt_scan_right_to_left_8u_omp(
  ...
  if (threads == 1 || n < 65536) {
    index =
        libsais_final_bwt_scan_right_to_left_8u(T, SA, induction_bucket, 0, n);
  }
#if defined(LIBSAIS_OPENMP)
  else {
    fast_sint_t block_start;
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          libsais_final_bwt_scan_right_to_left_8u_block_omp(
              T, SA, k, induction_bucket, block_end + 1, block_size, threads,
              thread_state);
          block_start = block_end;
        }
      }
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          libsais_final_bwt_aux_scan_right_to_left_8u_block_omp(
              T, SA, k, rm, I, induction_bucket, block_end + 1, block_size,
              threads, thread_state);
          block_start = block_end;
        }
      }
...
static void libsais_final_sorting_scan_right_to_left_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA,
    fast_sint_t omp_block_start, fast_sint_t omp_block_size, sa_sint_t k,
    sa_sint_t* RESTRICT induction_bucket, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  if (threads == 1 || omp_block_size < 65536) {
    ...
    fast_sint_t block_start;
    for (block_start = omp_block_start + omp_block_size - 1;
         block_start >= omp_block_start;) {
      if (SA[block_start] == 0) {
        block_start--;
      } else {
        fast_sint_t block_max_end =
...
static void libsais_final_gsa_scan_right_to_left_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA,
    fast_sint_t omp_block_start, fast_sint_t omp_block_size, sa_sint_t k,
    sa_sint_t* RESTRICT induction_bucket, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
    ...
    for (block_start = omp_block_start + omp_block_size - 1;
      ...
      if (SA[block_start] == 0) {
        ...
        if (block_size < 32) {
          ...
          libsais_final_gsa_scan_right_to_left_8u_block_omp(
              T, SA, k, induction_bucket, block_end + 1, block_size, threads,
              thread_state);
          block_start = block_end;
        }
...
static sa_sint_t libsais_induce_final_order_8u_omp(
    const uint8_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t k,
    sa_sint_t flags, sa_sint_t r, sa_sint_t* RESTRICT I,
    sa_sint_t* RESTRICT buckets, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  if ((flags & LIBSAIS_FLAGS_BWT) == 0) {
    ...
    libsais_final_sorting_scan_left_to_right_8u_omp(
        T, SA, n, k, &buckets[6 * ALPHABET_SIZE], threads, thread_state);
    if (threads > 1 && n >= 65536) {
      libsais_clear_lms_suffixes_omp(SA, n, ALPHABET_SIZE,
                                     &buckets[6 * ALPHABET_SIZE],
    ...
    if (flags & LIBSAIS_FLAGS_GSA) {
      ...
      libsais_final_gsa_scan_right_to_left_8u_omp(
          T, SA, buckets[7 * ALPHABET_SIZE],
          (fast_sint_t)n - buckets[7 * ALPHABET_SIZE], k,
          &buckets[7 * ALPHABET_SIZE], threads, thread_state);
    } else {
      libsais_final_sorting_scan_right_to_left_8u_omp(
          T, SA, 0, n, k, &buckets[7 * ALPHABET_SIZE], threads, thread_state);
    }

    return 0;
  } else if (I != NULL) {
    libsais_final_bwt_aux_scan_left_to_right_8u_omp(T, SA, n, k, r - 1, I,
                                                    &buckets[6 * ALPHABET_SIZE],
                                                    threads, thread_state);
    if (threads > 1 && n >= 65536) {
      libsais_clear_lms_suffixes_omp(SA, n, ALPHABET_SIZE,
    ...
    libsais_final_bwt_aux_scan_right_to_left_8u_omp(T, SA, n, k, r - 1, I,
                                                    &buckets[7 * ALPHABET_SIZE],
                                                    threads, thread_state);
    return 0;
  } else {
    ...
    return libsais_final_bwt_scan_right_to_left_8u_omp(
        T, SA, n, k, &buckets[7 * ALPHABET_SIZE], threads, thread_state);
  }
}
...
static void libsais_induce_final_order_32s_6k(
  ...
  libsais_final_sorting_scan_left_to_right_32s_omp(
      T, SA, n, &buckets[4 * (fast_sint_t)k], threads, thread_state);
  libsais_final_sorting_scan_right_to_left_32s_omp(
      T, SA, n, &buckets[5 * (fast_sint_t)k], threads, thread_state);
}
...
static void libsais_induce_final_order_32s_4k(
    ...
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  libsais_final_sorting_scan_left_to_right_32s_omp(
      T, SA, n, &buckets[2 * (fast_sint_t)k], threads, thread_state);
  libsais_final_sorting_scan_right_to_left_32s_omp(
      T, SA, n, &buckets[3 * (fast_sint_t)k], threads, thread_state);
}
...
static sa_sint_t libsais_renumber_unique_and_nonunique_lms_suffixes_32s(
  ...
  for (i = (sa_sint_t)omp_block_start, j = (sa_sint_t)omp_block_start +
    ...
    sa_sint_t* RESTRICT Tq0 = &T[q0];
    libsais_prefetchw(SAm[q0 >> 1] < 0 ? Tq0 : &SAm[q0 >> 1]);
    sa_uint_t q1 = (sa_uint_t)SA[i + prefetch_distance + 1];
    sa_sint_t* RESTRICT Tq1 = &T[q1];
    libsais_prefetchw(SAm[q1 >> 1] < 0 ? Tq1 : &SAm[q1 >> 1]);
    sa_uint_t q2 = (sa_uint_t)SA[i + prefetch_distance + 2];
    sa_sint_t* RESTRICT Tq2 = &T[q2];
    libsais_prefetchw(SAm[q2 >> 1] < 0 ? Tq2 : &SAm[q2 >> 1]);
    sa_uint_t q3 = (sa_uint_t)SA[i + prefetch_distance + 3];
    sa_sint_t* RESTRICT Tq3 = &T[q3];
    libsais_prefetchw(SAm[q3 >> 1] < 0 ? Tq3 : &SAm[q3 >> 1]);
...
static void libsais_compact_unique_and_nonunique_lms_suffixes_32s_omp(
    sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t m, sa_sint_t fs, sa_sint_t f,
    sa_sint_t threads, LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
#if defined(LIBSAIS_OPENMP)
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 131072 && \
                                                  m < fs)
...
#pragma omp master
      {
        ...
        for (position = m, t = omp_num_threads - 1; t >= 0; --t) {
          fast_sint_t omp_block_end = t < omp_num_threads - 1
                                          ...
                                          : ((fast_sint_t)n >> 1);
          fast_sint_t count = ((fast_sint_t)m + ((fast_sint_t)n >> 1) +
...
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_merge_unique_lms_suffixes_32s(T, SA, n, m, 0, omp_block_start,
                                            omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
...
#pragma omp barrier

      {
        ...
        libsais_merge_unique_lms_suffixes_32s(T, SA, n, m, count,
                                              omp_block_start, omp_block_size);
      }
    }
#endif
...
#pragma omp parallel num_threads(threads) if (threads > 1 && m >= 65536)
#endif
  {
    ...
    if (omp_num_threads == 1) {
      libsais_merge_nonunique_lms_suffixes_32s(SA, n, m, f, omp_block_start,
                                               omp_block_size);
    }
#if defined(LIBSAIS_OPENMP)
    else {
  ...
  if (k > 0 && ((fs / k >= 6) || (LIBSAIS_LOCAL_BUFFER_SIZE / k >= 6))) {
    ...
    if (m > 1) {
      ...
      libsais_radix_sort_lms_suffixes_32s_6k_omp(
          T, SA, n, m, &buckets[4 * (fast_sint_t)k], threads, thread_state);

      if ((n / 8192) < k) {
        libsais_radix_sort_set_markers_32s_6k_omp(
            SA, k, &buckets[4 * (fast_sint_t)k], threads);
      }
      if (threads > 1 && n >= 65536) {
      ...
      libsais_induce_partial_order_32s_6k_omp(
          T, SA, n, k, buckets, first_lms_suffix, left_suffixes_count, threads,
          thread_state);
      ...
      sa_sint_t names =
          (n / 8192) < k
              ...
              : libsais_renumber_and_gather_lms_suffixes_omp(
                    SA, n, m, fs, threads, thread_state);

      if (names < m) {
        sa_sint_t f = (n / 8192) < k
                          ? libsais_compact_lms_suffixes_32s_omp(
                                T, SA, n, m, fs, threads, thread_state)
                          : 0;

        if (libsais_main_32s_recursion(SA + n + fs - m + f, SA, m - f,
                                       names - f, fs + n - 2 * m + f, threads,
                                       thread_state, local_buffer) != 0) {
          return -2;
        }

        libsais_reconstruct_compacted_lms_suffixes_32s_2k_omp(
            T, SA, n, k, m, fs, f, buckets, buckets == local_buffer, threads,
            thread_state);
      } else {
        libsais_count_lms_suffixes_32s_2k(T, n, k, buckets);
      }
    ...
    return 0;
  } else if (k > 0 && (n <= SAINT_MAX / 2) &&
             ((fs / k >= 4) || (LIBSAIS_LOCAL_BUFFER_SIZE / k >= 4))) {
    sa_sint_t alignment =
        (fs - 1024) / k >= 4 ? (sa_sint_t)1024 : (sa_sint_t)16;
    sa_sint_t* RESTRICT buckets =
        (fs - alignment) / k >= 4
            ? (sa_sint_t*)libsais_align_up(
                  &SA[n + fs - 4 * (fast_sint_t)k - alignment],
                  (size_t)alignment * sizeof(sa_sint_t))
    ...
    if (m > 1) {
      ...
      libsais_induce_partial_order_32s_4k_omp(T, SA, n, k, buckets, threads,
                                              thread_state);
    ...
    sa_sint_t alignment =
        (fs - 1024) / k >= 2 ? (sa_sint_t)1024 : (sa_sint_t)16;
    sa_sint_t* RESTRICT buckets =
        (fs - alignment) / k >= 2
            ? (sa_sint_t*)libsais_align_up(
                  &SA[n + fs - 2 * (fast_sint_t)k - alignment],
                  (size_t)alignment * sizeof(sa_sint_t))
    ...
    if (m > 1) {
      ...
      libsais_induce_partial_order_32s_2k_omp(T, SA, n, k, buckets, threads,
                                              thread_state);
    ...
    if (m > 1) {
      libsais_induce_partial_order_32s_1k_omp(T, SA, n, k, buckets, threads,
                                              thread_state);
...
static sa_sint_t libsais_main_32s_entry(
    sa_sint_t* RESTRICT T, sa_sint_t* RESTRICT SA, sa_sint_t n, sa_sint_t k,
    sa_sint_t fs, sa_sint_t threads,
    LIBSAIS_THREAD_STATE* RESTRICT thread_state) {
  sa_sint_t local_buffer[2 * LIBSAIS_LOCAL_BUFFER_SIZE];

  return libsais_main_32s_recursion(T, SA, n, k, fs, threads, thread_state,
                                    local_buffer + LIBSAIS_LOCAL_BUFFER_SIZE);
}

static sa_sint_t libsais_main_8u(const uint8_t* T, sa_sint_t* SA, sa_sint_t n,
                                 sa_sint_t* RESTRICT buckets, sa_sint_t flags,
  ...
  if ((flags & LIBSAIS_FLAGS_GSA) &&
      (buckets[0] != 0 || buckets[2] != 0 || buckets[3] != 1)) {
    return -1;
  }
  ...
  if (m > 0) {
    ...
    libsais_radix_sort_lms_suffixes_8u_omp(T, SA, n, m, flags, buckets, threads,
                                           thread_state);
    if (threads > 1 && n >= 65536) {
      memset(&SA[(fast_sint_t)n - (fast_sint_t)m], 0,
             (size_t)m * sizeof(sa_sint_t));
    ...
    libsais_induce_partial_order_8u_omp(T, SA, n, k, flags, buckets,
                                        first_lms_suffix, left_suffixes_count,
                                        threads, thread_state);

    sa_sint_t names = libsais_renumber_and_gather_lms_suffixes_omp(
        SA, n, m, fs, threads, thread_state);
    if (names < m) {
      if (libsais_main_32s_entry(SA + n + fs - m, SA, m, names, fs + n - 2 * m,
  ...
  return libsais_induce_final_order_8u_omp(T, SA, n, k, flags, r, I, buckets,
                                           threads, thread_state);
}

static sa_sint_t libsais_main(const uint8_t* T, sa_sint_t* SA, sa_sint_t n,
                              sa_sint_t flags, sa_sint_t r, sa_sint_t* I,
                              sa_sint_t fs, sa_sint_t* freq,
                              sa_sint_t threads) {
  LIBSAIS_THREAD_STATE* RESTRICT thread_state =
      threads > 1 ? libsais_alloc_thread_state(threads) : NULL;
  ...
  sa_sint_t index = buckets != NULL && (thread_state != NULL || threads == 1)
                        ? libsais_main_8u(T, SA, n, buckets, flags, r, I, fs,
...
static sa_sint_t libsais_main_int(sa_sint_t* T, sa_sint_t* SA, sa_sint_t n,
                                  sa_sint_t k, sa_sint_t fs,
                                  sa_sint_t threads) {
  LIBSAIS_THREAD_STATE* RESTRICT thread_state =
      threads > 1 ? libsais_alloc_thread_state(threads) : NULL;
...
static sa_sint_t libsais_main_ctx(const LIBSAIS_CONTEXT* ctx, const uint8_t* T,
                                  sa_sint_t* SA, sa_sint_t n, sa_sint_t flags,
                                  sa_sint_t r, sa_sint_t* I, sa_sint_t fs,
                                  sa_sint_t* freq) {
  return ctx != NULL && (ctx->buckets != NULL &&
                         (ctx->thread_state != NULL || ctx->threads == 1))
             ? libsais_main_8u(T, SA, n, ctx->buckets, flags, r, I, fs, freq,
...
static void libsais_bwt_copy_8u_omp(uint8_t* RESTRICT U, sa_sint_t* RESTRICT A,
...
int32_t libsais_gsa(const uint8_t* T, int32_t* SA, int32_t n, int32_t fs,
                    int32_t* freq) {
  if ((T == NULL) || (SA == NULL) || (n < 0) || (n > 0 && T[n - 1] != 0) ||
      (fs < 0)) {
    return -1;
  } else if (n <= 1) {
    if (freq != NULL) {
...
int32_t libsais_ctx(const void* ctx, const uint8_t* T, int32_t* SA, int32_t n,
  ...
  return libsais_main_ctx((const LIBSAIS_CONTEXT*)ctx, T, SA, n,
                          LIBSAIS_FLAGS_NONE, 0, NULL, fs, freq);
}

int32_t libsais_gsa_ctx(const void* ctx, const uint8_t* T, int32_t* SA,
                        int32_t n, int32_t fs, int32_t* freq) {
  if ((ctx == NULL) || (T == NULL) || (SA == NULL) || (n < 0) ||
      (n > 0 && T[n - 1] != 0) || (fs < 0)) {
    return -1;
  } else if (n <= 1) {
    if (freq != NULL) {
  ...
  return libsais_main_ctx((const LIBSAIS_CONTEXT*)ctx, T, SA, n,
                          LIBSAIS_FLAGS_GSA, 0, NULL, fs, freq);
}
...
int32_t libsais_bwt(const uint8_t* T, uint8_t* U, int32_t* A, int32_t n,
  ...
  sa_sint_t index =
      libsais_main(T, A, n, LIBSAIS_FLAGS_BWT, 0, NULL, fs, freq, 1);
  if (index >= 0) {
    index++;
...
int32_t libsais_bwt_ctx(const void* ctx, const uint8_t* T, uint8_t* U,
  ...
  sa_sint_t index = libsais_main_ctx((const LIBSAIS_CONTEXT*)ctx, T, A, n,
                                     LIBSAIS_FLAGS_BWT, 0, NULL, fs, freq);
  if (index >= 0) {
    index++;
...
void* libsais_create_ctx_omp(int32_t threads) {
  ...
  threads =
      threads > 0 ? threads : (omp_get_max_threads() / omp_get_num_threads());
...
int32_t libsais_gsa_omp(const uint8_t* T, int32_t* SA, int32_t n, int32_t fs,
                        int32_t* freq, int32_t threads) {
  if ((T == NULL) || (SA == NULL) || (n < 0) || (n > 0 && T[n - 1] != 0) ||
      (fs < 0) || (threads < 0)) {
    return -1;
...
int32_t libsais_bwt_omp(const uint8_t* T, uint8_t* U, int32_t* A, int32_t n,
  ...
  sa_sint_t index =
      libsais_main(T, A, n, LIBSAIS_FLAGS_BWT, 0, NULL, fs, freq, threads);
  if (index >= 0) {
    index++;
...
int32_t libsais_bwt_aux_omp(const uint8_t* T, uint8_t* U, int32_t* A, int32_t n,
                            int32_t fs, int32_t* freq, int32_t r, int32_t* I,
                            int32_t threads) {
  if ((T == NULL) || (U == NULL) || (A == NULL) || (n < 0) || (fs < 0) ||
  ...
  sa_sint_t index =
      libsais_main(T, A, n, LIBSAIS_FLAGS_BWT, r, I, fs, freq, threads);
  if (index == 0) {
    U[0] = T[n - 1];
  ...
  for (sum = 1, c = 0; c < ALPHABET_SIZE; ++c) {
    ...
    if (prev != sum) {
...

      {
        ...
        libsais_unbwt_compute_histogram(&T[prev], (fast_sint_t)(hi - prev),
                                        bucket2_p);
      }

      {
        ...
        libsais_unbwt_compute_histogram(&T[lo - 1], (fast_sint_t)(sum - lo),
                                        bucket2_p);
      }
    }
  }
...
static void libsais_unbwt_calculate_biPSI(
    ...
    sa_uint_t* RESTRICT bucket1, sa_uint_t* RESTRICT bucket2, fast_uint_t index,
    fast_sint_t omp_block_start, fast_sint_t omp_block_end) {
  {
    ...
    for (; i < j; ++i) {
      ...
      if (t != 0) {
        fast_uint_t w =
            (((fast_uint_t)
                  T[p + (fast_uint_t)(t >> ((sizeof(fast_sint_t) * 8) - 1))])
             << 8) +
            c;
...
static void libsais_unbwt_init_single(const uint8_t* RESTRICT T,
  ...
  fast_uint_t shift = 0;
  while ((n >> shift) > ((sa_sint_t)1 << UNBWT_FASTBITS)) {
    shift++;
  }
...
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
  {
    ...
    if (omp_num_threads == 1) {
      ...
      sa_uint_t* RESTRICT bucket1_local =
          buckets +
          omp_thread_num * (ALPHABET_SIZE + (ALPHABET_SIZE * ALPHABET_SIZE));
...

      {
        ...
        libsais_unbwt_compute_histogram(T + omp_block_start, omp_block_size,
                                        bucket1_local);
      }
      ...
      {
        {
          ...
          fast_sint_t t;
          for (t = 0; t < omp_num_threads; ++t,
              bucket1_temp += ALPHABET_SIZE + (ALPHABET_SIZE * ALPHABET_SIZE)) {
            fast_sint_t c;
...
#pragma omp barrier

      {
        ...
        fast_sint_t t;
        for (t = 0; t < omp_num_threads; ++t,
            bucket2_temp += ALPHABET_SIZE + (ALPHABET_SIZE * ALPHABET_SIZE)) {
          fast_sint_t c;
          for (c = omp_bucket2_start; c < omp_bucket2_start + omp_bucket2_size;
               c += 1) {
...
static void libsais_unbwt_decode(uint8_t* RESTRICT U, sa_uint_t* RESTRICT P,
                                 ...
                                 sa_uint_t* RESTRICT bucket2,
                                 uint16_t* RESTRICT fastbits,
                                 fast_sint_t blocks, fast_uint_t remainder) {
  fast_uint_t shift = 0;
  while ((n >> shift) > ((sa_sint_t)1 << UNBWT_FASTBITS)) {
    shift++;
  }
  ...
  if (blocks == 1) {
    ...
    libsais_unbwt_decode_2(U + offset, P, bucket2, fastbits, shift,
                           (fast_uint_t)r, &i0, &i1, remainder >> 1);
    libsais_unbwt_decode_1(U + offset + 2 * (remainder >> 1), P, bucket2,
                           fastbits, shift, &i0,
                           ((fast_uint_t)r >> 1) - (remainder >> 1));
  } else if (blocks == 3) {
    fast_uint_t i0 = I[0], i1 = I[1], i2 = I[2];
    libsais_unbwt_decode_3(U + offset, P, bucket2, fastbits, shift,
                           (fast_uint_t)r, &i0, &i1, &i2, remainder >> 1);
    libsais_unbwt_decode_2(U + offset + 2 * (remainder >> 1), P, bucket2,
    ...
    libsais_unbwt_decode_4(U + offset, P, bucket2, fastbits, shift,
                           (fast_uint_t)r, &i0, &i1, &i2, &i3, remainder >> 1);
    libsais_unbwt_decode_3(U + offset + 2 * (remainder >> 1), P, bucket2,
    ...
    libsais_unbwt_decode_5(U + offset, P, bucket2, fastbits, shift,
                           ...
                           remainder >> 1);
    libsais_unbwt_decode_4(U + offset + 2 * (remainder >> 1), P, bucket2,
                           fastbits, shift, (fast_uint_t)r, &i0, &i1, &i2, &i3,
                           ((fast_uint_t)r >> 1) - (remainder >> 1));
  } else if (blocks == 6) {
    fast_uint_t i0 = I[0], i1 = I[1], i2 = I[2], i3 = I[3], i4 = I[4],
                i5 = I[5];
    libsais_unbwt_decode_6(U + offset, P, bucket2, fastbits, shift,
                           ...
                           remainder >> 1);
    libsais_unbwt_decode_5(U + offset + 2 * (remainder >> 1), P, bucket2,
                           fastbits, shift, (fast_uint_t)r, &i0, &i1, &i2, &i3,
                           &i4, ((fast_uint_t)r >> 1) - (remainder >> 1));
  } else if (blocks == 7) {
    fast_uint_t i0 = I[0], i1 = I[1], i2 = I[2], i3 = I[3], i4 = I[4],
                i5 = I[5], i6 = I[6];
    libsais_unbwt_decode_7(U + offset, P, bucket2, fastbits, shift,
                           (fast_uint_t)r, &i0, &i1, &i2, &i3, &i4, &i5, &i6,
                           remainder >> 1);
    libsais_unbwt_decode_6(U + offset + 2 * (remainder >> 1), P, bucket2,
                           fastbits, shift, (fast_uint_t)r, &i0, &i1, &i2, &i3,
                           &i4, &i5, ((fast_uint_t)r >> 1) - (remainder >> 1));
  } else {
    fast_uint_t i0 = I[0], i1 = I[1], i2 = I[2], i3 = I[3], i4 = I[4],
                i5 = I[5], i6 = I[6], i7 = I[7];
    libsais_unbwt_decode_8(U + offset, P, bucket2, fastbits, shift,
                           (fast_uint_t)r, &i0, &i1, &i2, &i3, &i4, &i5, &i6,
                           &i7, remainder >> 1);
    libsais_unbwt_decode_7(U + offset + 2 * (remainder >> 1), P, bucket2,
                           fastbits, shift, (fast_uint_t)r, &i0, &i1, &i2, &i3,
                           &i4, &i5, &i6,
                           ((fast_uint_t)r >> 1) - (remainder >> 1));
  }
...
static void libsais_unbwt_decode_omp(const uint8_t* RESTRICT T,
                                     uint8_t* RESTRICT U, sa_uint_t* RESTRICT P,
                                     sa_sint_t n, sa_sint_t r,
                                     const sa_uint_t* RESTRICT I,
                                     sa_uint_t* RESTRICT bucket2,
                                     uint16_t* RESTRICT fastbits,
...
#pragma omp parallel num_threads(max_threads) if (max_threads > 1 && n >= 65536)
#endif
  {
    ...
    fast_sint_t omp_block_size =
        omp_block_stride + (omp_thread_num < omp_block_remainder);
    fast_sint_t omp_block_start =
        omp_block_stride * omp_thread_num +
        (omp_thread_num < omp_block_remainder ? omp_thread_num
                                              : omp_block_remainder);
...
static sa_sint_t libsais_unbwt_core(const uint8_t* RESTRICT T,
                                    uint8_t* RESTRICT U, sa_uint_t* RESTRICT P,
  ...
  if (threads > 1 && n >= 262144) {
    libsais_unbwt_init_parallel(T, P, n, freq, I, bucket2, fastbits, buckets,
                                threads);
  } else
#else
  UNUSED(buckets);
...
static sa_sint_t libsais_unbwt_main(const uint8_t* T, uint8_t* U, sa_uint_t* P,
                                    sa_sint_t n, const sa_sint_t* freq,
...
int32_t libsais_unbwt_aux(const uint8_t* T, uint8_t* U, int32_t* A, int32_t n,
...
int32_t libsais_unbwt_aux_ctx(const void* ctx, const uint8_t* T, uint8_t* U,
  ...
  return libsais_unbwt_main_ctx((const LIBSAIS_UNBWT_CONTEXT*)ctx, T, U,
                                (sa_uint_t*)A, n, freq, r, (const sa_uint_t*)I);
}
...
int32_t libsais_unbwt_omp(const uint8_t* T, uint8_t* U, int32_t* A, int32_t n,
                          const int32_t* freq, int32_t i, int32_t threads) {
...
static void libsais_compute_plcp_gsa(const uint8_t* RESTRICT T,
                                     sa_sint_t* RESTRICT PLCP,
                                     fast_sint_t omp_block_start,
                                     fast_sint_t omp_block_size) {
  const fast_sint_t prefetch_distance = 64;
...
static void libsais_compute_plcp_gsa_omp(const uint8_t* RESTRICT T,
                                         sa_sint_t* RESTRICT PLCP, sa_sint_t n,
                                         sa_sint_t threads) {
#if defined(LIBSAIS_OPENMP)
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
...
static void libsais_compute_plcp_int(const int32_t* RESTRICT T,
                                     sa_sint_t* RESTRICT PLCP, fast_sint_t n,
                                     fast_sint_t omp_block_start,
                                     fast_sint_t omp_block_size) {
  const fast_sint_t prefetch_distance = 64;
...
static void libsais_compute_plcp_int_omp(const int32_t* RESTRICT T,
                                         sa_sint_t* RESTRICT PLCP, sa_sint_t n,
                                         sa_sint_t threads) {
#if defined(LIBSAIS_OPENMP)
#pragma omp parallel num_threads(threads) if (threads > 1 && n >= 65536)
...
int32_t libsais_plcp_gsa(const uint8_t* T, const int32_t* SA, int32_t* PLCP,
                         int32_t n) {
  if ((T == NULL) || (SA == NULL) || (PLCP == NULL) || (n < 0) ||
      (n > 0 && T[n - 1] != 0)) {
    return -1;
...
int32_t libsais_plcp_int(const int32_t* T, const int32_t* SA, int32_t* PLCP,
                         int32_t n) {
  if ((T == NULL) || (SA == NULL) || (PLCP == NULL) || (n < 0)) {
    return -1;
  } else if (n <= 1) {
...
int32_t libsais_plcp_gsa_omp(const uint8_t* T, const int32_t* SA, int32_t* PLCP,
                             int32_t n, int32_t threads) {
  if ((T == NULL) || (SA == NULL) || (PLCP == NULL) || (n < 0) ||
      (n > 0 && T[n - 1] != 0) || (threads < 0)) {
    return -1;
  } else if (n <= 1) {
    if (n == 1) {
      PLCP[0] = 0;
...
int32_t libsais_plcp_int_omp(const int32_t* T, const int32_t* SA, int32_t* PLCP,
                             int32_t n, int32_t threads) {
  if ((T == NULL) || (SA == NULL) || (PLCP == NULL) || (n < 0) ||
      (threads < 0)) {
    return -1;
