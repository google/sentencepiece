 ...
 *
 * @brief bs::ThreadPool: a fast, lightweight, and easy-to-use C++17 thread pool library. This header file contains a light version of the main library, for use when advanced features are not needed.
 */
...
public:
    ...
    /**
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue. The user must use Wait() or some other method to ensure that the loop finishes executing, otherwise bad things will happen.
     *
    ...
    void Loop(T1 first_index_, T2 index_after_last_, F&& loop, size_t num_blocks = 0)
    {
        ...
        if (total_size > 0)
        {
            for (size_t i = 0; i < num_blocks; ++i)
                Schedule(std::forward<F>(loop), static_cast<T>(i * block_size) + first_index, (i == num_blocks - 1) ? index_after_last : (static_cast<T>((i + 1) * block_size) + first_index));
        }
    }

    /**
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block separately to the queue. The user must use Wait() or some other method to ensure that the loop finishes executing, otherwise bad things will happen. This overload is used for the special case where the first index is 0.
     *
     * @tparam F The type of the function to loop through.
    ...
    void Loop(const T index_after_last, F&& loop, const size_t num_blocks = 0)
    {
        Loop(0, index_after_last, std::forward<F>(loop), num_blocks);
    }

    /**
     * @brief Push a function with zero or more arguments, but no return value, into the task queue. Does not return a future, so the user must use Wait() or some other method to ensure that the task finishes executing, otherwise bad things will happen.
     *
     ...
     * @param task The function to submit to the thread pool for execution.
     * @param args The zero or more arguments to pass to the function. Note that if the task is a class member function, the first argument must be a pointer to the object, i.e. &object (or this), followed by the actual arguments.
...
private:
    ...
    /**
     * @brief A worker function to be assigned to each thread in the pool. Waits until it is notified by Schedule() that a task is available, and then retrieves the task from the queue and executes it. Once the task finishes, the worker notifies Wait() in case it is waiting.
     */
    void worker()
    {
        while (running)
    ...
    /**
     * @brief A condition variable used to notify Wait() that a tasks is done.
     */
    ...
    /**
     * @brief An atomic variable indicating that Wait() is active and expects to be notified whenever a task is done.
     */
