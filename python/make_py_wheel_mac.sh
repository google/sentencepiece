...
build_python() {
  ...
  python get-pip.py --no-setuptools --no-wheel --ignore-installed
...
build() {
  ...
  make CXXFLAGS+="-std=c++11 -O3 -DGOOGLE_PROTOBUF_NO_THREADLOCAL=1"  CFLAGS+="-std=c++11 -O3 -DGOOGLE_PROTOBUF_NO_THREADLOCAL=1" -j4
  make install || true
  cd ..
