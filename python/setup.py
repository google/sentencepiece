...
def read_version():
  ...
  version of the C++ library, the Bazel module and this Python package.
  When building from a source distribution produced by build_sdist.py, the
  file lives under ./sentencepiece/VERSION.txt instead.
  """
  here = os.path.dirname(os.path.abspath(__file__))
  candidates = [
      os.path.join(here, '..', 'VERSION.txt'),
      os.path.join(here, 'sentencepiece', 'VERSION.txt'),
  ]
  for path in candidates:
    if os.path.exists(path):
      ...
      if version:
        return version
  raise RuntimeError(
      'VERSION.txt not found (searched: {})'.format(', '.join(candidates)))


def write_version_module(version):
  """Generates src/sentencepiece/_version.py from VERSION.txt."""
  here = os.path.dirname(os.path.abspath(__file__))
  path = os.path.join(here, 'src', 'sentencepiece', '_version.py')
  content = (
      '# Generated by setup.py from VERSION.txt. Do not edit.\n'
      "__version__ = '{}'\n".format(version)
  )
  if os.path.exists(path):
    with open(path, encoding='utf-8') as f:
...
write_version_module(__version__)
